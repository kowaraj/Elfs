...
(comment
(def tdim 3)
(defstruct tcell :food :pher)
(defstruct tant :dir)
(def tw (apply vector (map (fn [_] (apply vector (map (fn[_] (ref (struct tcell 0 0))) (range tdim)))) (range tdim))))
(defn tpl [[x y]] (-> tw (nth x) (nth y)))
(defn tca [loc dir] (let [p (tpl loc) a (struct ant dir)] (alter p assoc :ant a) (agent loc)))
(defn ts [] (sync nil (doall (for [x '(1) y '(1)] (do (alter (tpl [x y]) assoc :home true) (tca [x y] (rand-int 8)))))))
(def tants (ts))

;demo
(load-file "/home/kowaraj/usr/src/elfs/ants.clj")
(def ants (setup))
